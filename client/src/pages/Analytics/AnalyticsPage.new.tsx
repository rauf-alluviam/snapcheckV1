...
const AnalyticsPage: React.FC = () => {
  ...
  const inspectorData: ChartData<'bar'> = {
    labels: analyticsData?.inspectorPerformance.map((i: any) => i.name) || [],
    datasets: [
      {
        label: 'Inspections Completed',
        data: analyticsData?.inspectorPerformance.map((i: any) => i.completed) || [],
        backgroundColor: 'rgba(37, 99, 235, 0.7)',
      },
      {
        label: 'Approval Rate (%)',
        data: analyticsData?.inspectorPerformance.map((i: any) => i.approvalRate) || [],
  ...
  const completionTimeData: ChartData<'bar'> = {
    labels: Object.keys(analyticsData?.avgCompletionTime || {}),
    datasets: [
      {
        label: 'Average Completion Time (hours)',
        data: Object.values(analyticsData?.avgCompletionTime || {}),
